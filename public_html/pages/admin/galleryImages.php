...
/////////////////			FIELDS DEFINITION
				...
				// parent_cat
				{
					...
					"render": function ( data, type, full, meta ) {
                      if(data)
                          return 	'<img src="<?=site_config::get_value("upload_thumbs_folder")?>'+data+'" style="max-height: 60px;"/>';
                      else
                          return '';
					}
				},
				...
				<div class="form">
					<form id="formAddEditNewRow" method="post" action="" enctype="multipart/form-data"  role="form">
						...
						<div class="form-group" >
							...
							<input type="text" class="form-control" id="link" name="link" placeholder="קישור" value="<?=$this->get_edited_data("link")?>">
						</div>
						<div class="form-group" >
							<label for="video">קוד לוידאו YouTube</label>
							<input type="text" class="form-control" id="video" name="video" placeholder="(v=) קוד לוידאו YouTube" value="<?=$this->get_edited_data("video")?>">
						</div>
						<div class="form-group" >
							<div>
						...
						<div class="form-group" >
							<label for="desc">טקסט</label>
							<textarea class="form-control editor" id="desc" name="desc" placeholder="טקסט"><?=$this->get_edited_data("desc")?></textarea>
						</div>
						<div class="form-group" >
						...
						<input type="hidden" name="gal_id" value="<?=(int)$_GET["gal_id"]?>"/>
