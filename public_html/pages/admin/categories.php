			...
			<div class="panel-body">
				<div class="form">
					<form id="formAddEditNewRow" method="post" action="" enctype="multipart/form-data"  role="form">
						...
						<div class="form-group" >
							<label for="area_id">תחום</label>
							<select name="area_id" class="form-control">
								<option value="0">בחר תחום</option>
								<?
								$areas = db_con::get_con()->get_table('category_areas');
								if (!emptY($areas['rows'])){
									foreach($areas['rows'] as $area){
									?>
									<option value="<?=$area['id']?>" <?=$this->is_selected("area_id",$area['id'])?>><?=$area['name']?></option>
									<?
									}
								}
								?>
						...
						<div class="form-group" >
							<div>
								<label for="cat_image">תמונת קטגוריה</label>
							</div>
							<?
							$file_name = $this->get_edited_data("cat_image");
							if (!empty($file_name)){
							?>
								<div class="admin_img_container" style="float: right; margin-left: 10px;">
									<image src="<?=site_config::get_value('upload_thumbs_folder').$file_name?>" style="height: 70px;"/>
								...
								<div style="float: right; margin-bottom: 20px;">
									<button type="button" class="delete_image_btn" data-image_name="cat_image">
										מחק תמונה
									</button>
								</div>
							<?
						...
						<div class="form-group" >
							<div>
								<label for="side_image">תמונת לטפרית צד</label>
							</div>
							<?
							$file_name = $this->get_edited_data("side_image");
							if (!empty($file_name)){
							?>
								<div class="admin_img_container" style="float: right; margin-left: 10px;">
									<image src="<?=site_config::get_value('upload_thumbs_folder').$file_name?>" style="height: 70px;"/>
								...
								<div style="float: right; margin-bottom: 20px;">
									<button type="button" class="delete_image_btn" data-image_name="side_image">
										מחק תמונה
									</button>
								</div>
							<?
							...
							?>
							<input type="file" id="side_image" name="side_image" />
						</div>
						<div class="clear"></div>
						<div class="form-group" >
							<div>
						...
						<div class="form-group">
							...
							<div class="col-md-5">
								<div class="checkbox">
									<label>
										<input type='hidden' value='0' name='show_on_side_menu'>
										<input type="checkbox" name="show_on_side_menu" value="1" <?=$this->is_checked("show_on_side_menu",1)?>> הצג בטפרית צד
									</label>
								</div>
							</div>
							<div class="clearfix"></div>
