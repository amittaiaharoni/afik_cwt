...
////////////////////////////////////////////////////////////////////////
		...
		$("input[name=multi]").on("change",function(e){
			if($(this).is(":checked")){
				$("#expiration_date").parent("div.form-group").hide();
			}else{
				$("#expiration_date").parent("div.form-group").show();
			}
		});
	});
</script>
		...
		<div id="search_form_container" class="form well" style="display: none;">
			<form id="search_form" action="" method="post" role="form">
				<div class="form-group" >
					<div class="checkbox">
						<label>
							<input type="checkbox" name="giftcards" value="1"/>
							הצג גיפטקארדים
						</label>
					</div>
				</div>
			...
			<div class="panel-body">
				<div class="form">
					<form id="formAddEditNewRow" method="post" action="" enctype="multipart/form-data"  role="form">
						...
						<div class="form-group" >
							<label>מספר קופון : <?=$this->get_edited_data("number")?></label>
						</div>
							<?
							}
							else{
						...
						<div class="form-group" >
							...
							<input type="text" class="form-control" id="quantity" name="quantity" placeholder="כמות" value="<?=$this->get_edited_data("quantity")?>">
						</div>
							<?
							}
						?>
						...
						<div class="form-group" >
							<div class="checkbox">
								<label>
									...
									<input type="checkbox" name="used" value="1" <?=$this->is_checked("used",1,0)?>>
									נוצל
								</label>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div style="margin-right: 20px; float: right;">
								<label class="radio-inline">
								  <input type="radio" name="type" id="type" value="0" <?=$this->is_checked("type",0,0)?>> הנחה %
								</label>
							</div>
							<div style="margin-right: 50px; float: right;">
								<label class="radio-inline">
								  <input type="radio" name="type" id="type" value="1" <?=$this->is_checked("type",1,0)?>> סכום קבוע
								</label>
							</div>
							<div class="clearfix"></div>
						...
						<hr style="border-color: #aaa;"/>
						<div class="form-group" >
							<label for="amount">סכום\הנחה</label>
							<input type="text" class="form-control" id="amount" name="amount" placeholder="סכום\הנחה" value="<?=$this->get_edited_data("amount")?>">
						</div>
						<div class="form-group" >
							<div class="checkbox">
								<label>
									<input type='hidden' value='0' name='multi'>
									<input type="checkbox" name="multi" value="1" <?=$this->is_checked("multi",1,0)?>>
								  לא חדש פעמי
								</label>
							</div>
						</div>
						<div class="form-group" >
							...
							<input type="text" class="form-control datepicker" id="expiration_date" name="expiration_date" placeholder="תאריך תפוגה" value="<?=$this->get_edited_data("expiration_date")?>">
						</div>
						<div class="form-group" >
							<div class="checkbox">
								<label>
									<input type='hidden' value='0' name='is_giftcard'>
									<input type="checkbox" name="is_giftcard" value="1" <?=$this->is_checked("is_giftcard",1,0)?>>
									GIFTCARD
								</label>
							</div>
						</div>
