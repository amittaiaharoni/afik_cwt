...
/////////////////			FIELDS DEFINITION
				...
				{ "data": "id" },
				// name
				{ "data": "first_name" , "sClass": "editable"},
				{ "data": "last_name" , "sClass": "editable"},
				{ "data": "username" , "sClass": "editable"},

				{
					...
					"render":
						function ( data, type, full, meta ) {
							if (data)
								return 	'<a href="index.php?page=admin_orders&user_id='+data+'">להזמנות</a> ';
							else
								return "";
						}
				},
...
////////////////////////////////////////////////////////////////////////
		...
		$("#send_mail_cart").on("click",function(){ // ,#send_mail_cart$("select[name='send_mail_cart']")
			// var id = $(this).val();
			// if(id > 0){
				$.ajax({
					type: 'post',
...
<div id="page_wraper" class="container-fluid" style="position: relative;">
	...
	<div class="text-center col-md-4 col-md-offset-4" style="position: absolute; left: 0; top: -20px; z-index: 1000; background: white; border: 1px solid #333; border-radius: 7px; padding: 10px;">
		<div id="search_form_container" class="form well" style="display: none;">
			...
			<form action="" method="post" role="form">
				<div class="form-group" >
                    <input type="submit" name="birthday_mail" value="שלח הודעה לאנשי ימי הולדת"/>
				</div>
			</form>
		</div>
	...
	<div>
		<button id="send_mail_cart" class="btn btn-default">שלח מייל לכל המשתמשים שיש להם סל קניות שלא ביצע עסקה</button>
		<!--<select name="send_mail_cart">
			<option value="0">בחר שבלונת מייל</option>
			<?
			$mails = $this->data->mail->get_all();
			if(!empty($mails)){
				...
				<div class="form">
					<form id="formAddEditNewRow" method="post" action="" enctype="multipart/form-data"  role="form">
						<div class="form-group" >
							...
							<input type="text" class="form-control" id="first_name" name="first_name" placeholder="שם" value="<?=$this->get_edited_data("first_name")?>">
						</div>
						<div class="form-group" >
							<label for="last_name">שם משפחה</label>
							<input type="text" class="form-control" id="last_name" name="last_name" placeholder="שם משפחה" value="<?=$this->get_edited_data("last_name")?>">
						</div>
						<div class="form-group" >
							<label for="birthday">תאריך לידה</label>
							<input type="text" class="form-control datepicker colors" id="birthday" name="birthday" placeholder="תאריך לידה" value="<?=$this->get_edited_data("birthday")?>">
						</div>
						<div class="form-group" >
						...
						<div class="form-group" >
							...
							<input type="text" class="form-control" id="username" name="username" placeholder="שם משתמש" value="<?=$this->get_edited_data("username")?>">
						</div>
						<div class="form-group" >
							<label for="pass">סיסמה</label>
							<input type="text" class="form-control" id="pass" name="pass" placeholder="סיסמה" value="<?=$this->get_edited_data("pass")?>">
						</div>
						<div class="form-group" >
							<label for="credits">קרדיטים</label>
							<input type="text" class="form-control" id="credits" name="credits" placeholder="קרדיטים" value="<?=$this->get_edited_data("credits")?>">
						</div>
