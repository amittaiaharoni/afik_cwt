...
<div id="page_wraper" class="container-fluid" style="position: relative;">
		...
		<div id="search_form_container" class="form well" style="display: none;">
			<form id="search_form" action="" method="post" role="form">
				<div class="form-group" >
					<label for="cat_id">קטגוריית אב</label>
				...
				<div class="form">
					<form id="formAddEditNewRow" method="post" action="" enctype="multipart/form-data"  role="form">
						...
						<div class="form-group" >
							<label for="cat_id">קטגוריית אב</label>
							<select name="cat_id" class="form-control">
								<option value="0">בחר קטגוריית</option>
								<?
								$cats = $this->data->infoCat->get_all();
								if (!emptY($cats)){
									foreach($cats as $cat){
										$cat_parent_path = $this->data->infoCat->get_parent_cat_name($cat, true);
									?>
									<option value="<?=$cat->id?>" <?=$this->is_selected("cat_id",$cat->id)?>><?=$cat_parent_path." -> ".$cat->name?></option>
									<?
									}
								}
								?>
                        ...
                        <div class="checkbox">
                            <label>
                                <input type='hidden' value='0' name='show_in_footer_menu'>
                                <input type="checkbox" name="show_in_footer_menu" value="1" <?=$this->is_checked("show_in_footer_menu",1)?>>
                                להציג תחתית העמוד
                            </label>
                        </div>
						<div class="clear"></div>
						...
						<div class="form-group" >
							<div class="col-md-4">
								<div>
									<label for="pdf_file">קובץ מצורף 1</label>
								</div>
								<?
								$file_name = $this->get_edited_data("pdf_file");
								if (!empty($file_name)){
								?>
									<a href="<?=site_config::get_value('upload_files_folder').$file_name?>"><i class="fa fa-fw fa-table"></i>להורדה</a>
									<a href="#" class="delete_file_btn" data-file_name="pdf_file">מחק קובץ</a>
								<?
								}
								?>
								<input type="text" class="form-control" id="pdf_file" name="pdf_file" placeholder="שם קובץ" value="<?=$this->get_edited_data("pdf_file")?>">
								<br/>
						...
						<div class="clear"></div>
						<div class="form-group" >
							<textarea name="text" class="form-control editor"><?=$this->get_edited_data("text")?></textarea>
						</div>
						<hr style="border-color: #aaa;"/>
						<div class="form-group" >
	...
	<div id="" class="col-md-5">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h3 class="panel-title">רשימת דפי מידע</h3>
			</div>
			<div class="panel-body">
				<div class="table">
